...
const StaticTable = function <T extends object>(props: TableViewProps<T>) {
  return (
    <TableView
      data-id="table"
    ...
    >
      <TableHeader>
        <Column isRowHeader data-id="table-column">
          First Name
        </Column>
        <Column isRowHeader data-id="table-column">
          Last Name
        </Column>
        <Column align="end" data-id="table-column">
          City
        </Column>
      </TableHeader>
      <TableBody>
        <Row key="1">
          <Cell data-id="table-cell">Gary</Cell>
          <Cell data-id="table-cell">Ford</Cell>
          <Cell data-id="table-cell">London</Cell>
        </Row>
        <Row key="2">
          <Cell data-id="table-cell">Mark</Cell>
          <Cell data-id="table-cell">Lexus</Cell>
          <Cell data-id="table-cell">Bristol</Cell>
        </Row>
        <Row key="3">
          <Cell data-id="table-cell">Tamas</Cell>
          <Cell data-id="table-cell">Bentley</Cell>
          <Cell data-id="table-cell">London</Cell>
        </Row>
      </TableBody>
    </TableView>
...
describe("TableView", () => {
  describe("Static", () => {
    ...
    it("should select and deselect rows", () => {
      cy.mount(<StaticTable />);
      cy.get(`tr:nth-child(1) > td${input}`).check();
      cy.get(`${table} tbody tr:nth-child(1)`)
        .should("have.attr", "aria-selected", "true")
        .should("have.attr", "class")
        .and("to.have.string", "isSelected")
        .get(`tr:nth-child(1) > td${input}`)
        .should("be.checked");
      cy.get(`tr:nth-child(3) > td${input}`).check();
      cy.get(`tr:nth-child(3) > td${input}`).should("be.checked");
      cy.get(`tbody tr:nth-child(1) > td${input}`).check();
    });

    it("onRowAction fires correctly", () => {
      const onRowActionSpy = cy.spy().as("onRowActionSpy");
      cy.mount(<StaticTable onRowAction={(key) => onRowActionSpy(key)} />);
      cy.get(`tbody tr:nth-child(1)`).click();
      cy.get("@onRowActionSpy").should("have.been.calledWith", "1");
    });
  });
  ...
  describe("Dynamic", () => {
    ...
    it("onRowAction fires correctly", () => {
      const onRowActionSpy = cy.spy().as("onRowActionSpy");
      cy.mount(<DynamicTable onRowAction={(key) => onRowActionSpy(key)} />);
      cy.get(`tbody tr:nth-child(1)`).click();
      cy.get("@onRowActionSpy").should("have.been.calledWith", 1);
    });
  });
