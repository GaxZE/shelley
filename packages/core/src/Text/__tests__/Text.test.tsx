...
describe("H", () => {
  test("H1", () => {
    const tree = renderer.create(<H1>{shortText}</H1>).toJSON();
    expect(tree).toMatchSnapshot();
  });
  test("H2", () => {
    const tree = renderer.create(<H2>{shortText}</H2>).toJSON();
    expect(tree).toMatchSnapshot();
  });
  test("H3", () => {
    const tree = renderer.create(<H3>{shortText}</H3>).toJSON();
    expect(tree).toMatchSnapshot();
  });
  test("H3 volume", () => {
    const tree = renderer.create(<H3 vol={5}>{shortText}</H3>).toJSON();
    expect(tree).toMatchSnapshot();
  });
  test("H4", () => {
    const tree = renderer.create(<H4>{shortText}</H4>).toJSON();
    expect(tree).toMatchSnapshot();
  });
  test("H5", () => {
    const tree = renderer.create(<H5>{shortText}</H5>).toJSON();
    expect(tree).toMatchSnapshot();
  });
  test("H6", () => {
    const tree = renderer.create(<H6>{shortText}</H6>).toJSON();
    expect(tree).toMatchSnapshot();
  });
  test("H uppercase", () => {
    const tree = renderer.create(<H1 uppercase>{shortText}</H1>).toJSON();
    expect(tree).toMatchSnapshot();
  });
});
...
describe("P", () => {
  test("P", () => {
    const tree = renderer.create(<P>{shortText}</P>).toJSON();
    expect(tree).toMatchSnapshot();
  });
  test("P volume", () => {
    const tree = renderer.create(<P vol={5}>{shortText}</P>).toJSON();
    expect(tree).toMatchSnapshot();
  });
  test("P vol weight", () => {
    const tree = renderer
  ...
  test("P truncate", () => {
    const tree = renderer.create(<P truncate={2}>{longText}</P>).toJSON();
    expect(tree).toMatchSnapshot();
  });
});
      ...
      .create(
        <Text as="div" truncate={6} vol={false}>
          ...
          <P>
            ...
            interconnected with other components of the climate system through
